...
(ns typed.test.mini-kanren
  (:import (clojure.lang IMeta Symbol Keyword IPersistentList IPersistentMap
                         Seqable))
  (:require [typed.test.monads :refer [defmonad with-monad m-chain]]
            [clojure.pprint :refer [pprint]]
            [clojure.repl :refer [pst]]
            [typed.core :refer [ann inst def-alias check-ns declare-names
                                tc-ignore]]))
...
(defn- build-questions [c-list]
  (map #(cons 'typed.test.mini-kanren/test-question %)
       (remove-else c-list)))
