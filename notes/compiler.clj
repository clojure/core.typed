...
(new-type NsEntry
          (Map :mandatory
               {:name Symbol}
               :optional
               {:defs (Map Symbol Symbol)
                :uses (Map Symbol Symbol)
                :excludes (Set Symbol)}))
...
(: cljs-reserved-file-names (Set String))
(def cljs-reserved-file-names #{"deps.cljs"})

(: namespaces (Atom (Map Symbol NsEntry)))
(defonce namespaces (atom '{cljs.core {:name cljs.core}
                            cljs.user {:name cljs.user}}))

(: reset-namespaces! [-> Any])
...
(: confirm-var-exists [Env Any Any])
...
(: resolve-ns-alias [Env (U Symbol String) -> (U nil Symbol)])
(defn resolve-ns-alias [env name]
  (let [sym (symbol name)]
    (get (:requires (:ns env)) sym sym)))

(: core-name? [Env Symbol -> Any])
(defn core-name?
...
(: resolve-existing-var [Env Symbol -> LocalBinding])
...
(: confirm-bindings [Env (Seqable Symbol)])
...
(: comma-sep [(Seqable Any) -> (Seqable Any)])
(defn- comma-sep [xs]
  (interpose "," xs))

(: escape-char [Character -> Number])
...
(: escape-string [CharSequence -> String])
...
(: wrap-in-double-quotes [String -> String])
...
(: analyze-block [Env (Seqable Any) -> (Map :mandatory
...
(: analyze-fn-method [Env LocalBinding (Seqable Any) -> (Map :mandatory
                                                             {:env Env
                                                              :variadic Any
                                                              :params (Seqable Any)
                                                              :max-fixed-arity ])
...
(: forms-seq (Fn [String -> (Seqable Any)]
                 [String PushbackReader -> (Seqable Any)]))
(defn forms-seq
...
(defmacro with-core-cljs
  ...
  `(do (when-not (:defs (get @namespaces 'cljs.core))
       ...
       ~@body))
(+T compile-file* [File File -> NsAnalysis])
...
(: requires-compilation? [File File -> (U true false)])
