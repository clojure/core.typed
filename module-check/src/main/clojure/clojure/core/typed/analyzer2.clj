  ...
  (let [formc (count form)]
    (when-not (or (= formc 3) (= formc 4))
      (throw (ex-info (str "Wrong number of args to if, had: " (dec (count form)))
                      (merge {:form form}
                             (u/-source-info form env))))))
  (let [test-expr (analyze-form test (u/ctx env :ctx/expr))
...
(defn parse-set!
  ...
  (when-not (= 3 (count form))
    (throw (ex-info (str "Wrong number of args to set!, had: " (dec (count form)))
                    (merge {:form form}
                           (u/-source-info form env)))))
  (let [target (analyze-form target (u/ctx env :ctx/expr))
        val (analyze-form val (u/ctx env :ctx/expr))]
    {:op       :set!
     :env      env
     :form     form
     :target   target
...
(defn parse-throw
  ...
  {:op        :throw
   :env       env
   :form      form
   :exception (analyze-form throw (u/ctx env :ctx/expr))
   :children  [:exception]})
...
(defn parse-def
  ...
  (let [pfn (fn
        ...
        meta-expr (when meta (analyze-form meta (u/ctx env :ctx/expr))) ;; meta on def sym will be evaluated

        args (when-let [[_ init] (find args :init)]
               (assoc args :init (analyze-form init (u/ctx env :ctx/expr))))
        init? (:init args)
...
(defn parse-dot
  ...
  (let [[m-or-f field?] (if (and (symbol? m-or-f)
        ...
        target-expr (analyze-form target (u/ctx env :ctx/expr))
    ...
    (merge {:form   form
           ...
           (cond
            ...
            {:op       :host-call
             ...
             :args     (mapv (analyze-in-env (u/ctx env :ctx/expr)) (next m-or-f))
