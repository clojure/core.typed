...
(ann foo-kw [& :optional {:a Number} -> (U nil Number)])
(defn foo-kw [& {:keys [a]}]
  (when a
    (inc a)))
...
(ann foo-arg-kw [Number & :optional {:a Number} -> (U nil Number)])
...
(ann test-non-value-kw [& :optional {:a [Any -> Any]} -> Any])
...
(ann test-complete-hmap [& :optional {:a Number} -> Any])
