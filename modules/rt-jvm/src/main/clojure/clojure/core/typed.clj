...
for checking namespaces, cf for checking individual forms."}
  ...
  (:require [clojure.pprint :as pprint]
            ...
            [clojure.core.typed.chk.common.util-vars :as uvars]
...
(defn method-type
  ...
  (let [type-reflect (impl/v 'clojure.reflect/type-reflect)
        unparse-type (impl/v 'clojure.core.typed.chk.common.parse-unparse/unparse-type)
        Method->Type (impl/v 'clojure.core.typed.chk.common.check/Method->Type)
...
(defn check-form*
  ...
  ([form expected type-provided?]
   (load-if-needed)
   (let [unparse-TCResult-in-ns (impl/v 'clojure.core.typed.chk.common.parse-unparse/unparse-TCResult-in-ns)
...
(defn load-if-needed 
  ...
  (when-not *currently-loading*
    (binding [*collect-on-eval* false
      ...
      (when-not (find-ns 'clojure.core.typed.chk.common.init)
        (require 'clojure.core.typed.chk.common.init))
      (let [init-ns (find-ns 'clojure.core.typed.chk.common.init)]
        (assert init-ns)
...
(defn reset-caches 
  ...
  (load-if-needed)
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.subtype) 'reset-subtype-cache))
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.type-ctors) 'reset-Un-cache))
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.type-ctors) 'reset-In-cache))
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.type-ctors) 'reset-supers-cache!))
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.type-ctors) 'reset-RClass-of-cache!))
  (@(ns-resolve (find-ns 'clojure.core.typed.chk.common.cs-gen) 'reset-dotted-var-store!))
  nil))
...
(defn check-form-info 
  ...
  (p/profile-if profile
    (load-if-needed)
    (reset-caches)
    (let [check (impl/v 'clojure.core.typed.chk.common.check/check)
          expr-type (impl/v 'clojure.core.typed.chk.common.check/expr-type)
   ...
   (p/profile-if profile
     (let [start (. System (nanoTime))]
       ...
       (let [reset-envs! (impl/v 'clojure.core.typed.chk.common.reset-env/reset-envs!)
             ...
             check-ns-and-deps (impl/v 'clojure.core.typed.chk.common.check/check-ns-and-deps)
                    ...
                    [nsym
                     {:vars {:all-vars (all-defs-in-ns ns)
                             :no-checks (let [; deref the atom
                                              all-no-checks @(impl/v 'clojure.core.typed.chk.common.var-env/CLJ-NOCHECK-VAR?)]
                                          (filter (fn [s] (= (namespace s) nsym)) all-no-checks))
                             :var-annotations (let [; deref the atom
                                                    annots @(impl/v 'clojure.core.typed.chk.common.var-env/CLJ-VAR-ANNOTATIONS)]
                                                (->> annots
                                                     (filter (fn [[k v]] (= (namespace k) (str nsym))))
                                                     (map (fn [[k v]] [k (binding [*verbose-types* true]
                                                                           ((impl/v 'clojure.core.typed.chk.common.parse-unparse/unparse-type)
                                                                            v))]))
                                                     (into {})))}}])))
          {} nsyms))
...
(defmacro pred 
  ...
  eg. ((pred Number) 1)
      ;=> true"
  [t]
  (require '[clojure.core.typed.rt.jvm.type-contract])
  `(pred* '~t
          '~(ns-name *ns*)
          ~((impl/v 'clojure.core.typed.rt.jvm.type-contract/type-syntax->pred) t)))
