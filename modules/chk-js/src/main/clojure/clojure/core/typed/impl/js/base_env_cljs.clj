...
(defn reset-protocol-env! []
  (impl/with-cljs-impl
    ((impl/v 'clojure.core.typed.chk.common.protocol-env/reset-protocol-env!) 
     (init-protocol-env))))
...
(defn reset-jsnominal-env! []
  (impl/with-cljs-impl
    ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/reset-jsnominal!) 
     (init-jsnominals))))
    ...
    (assert (= (set (keys alias-env))
            ...
            (str "core.typed Bug! Base aliases do not agree with base environment."
                 ...
                 (set/difference (set (map #(symbol "cljs.core.typed" (str %))
                                           boot/-base-aliases))
                                 (set (keys alias-env)))))
    ((impl/v 'clojure.core.typed.chk.common.name-env/reset-name-env!) alias-env)))
...
(defn reset-cljs-envs! []
  (env/ensure
    (impl/with-cljs-impl
      (reset-alias-env!)
      ((impl/v 'clojure.core.typed.chk.common.var-env/reset-var-type-env!)
       (init-var-env) (init-var-nochecks))
      ((impl/v 'clojure.core.typed.chk.common.var-env/reset-jsvar-type-env!)
       (init-jsvar-env))
      (reset-protocol-env!)
      ((impl/v 'clojure.core.typed.chk.common.declared-kind-env/reset-declared-kinds!) 
       (init-declared-kinds))
      (reset-jsnominal-env!)
      ((impl/v 'clojure.core.typed.chk.common.datatype-env/reset-datatype-env!) 
       (init-datatype-env))))
  nil)
