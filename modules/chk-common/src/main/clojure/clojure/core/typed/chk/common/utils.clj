(ns ^:skip-wiki clojure.core.typed.chk.common.utils
  ...
  (:require [clojure.core.typed :as t]
            ...
            [clojure.core.typed.rt.common.profiling :as profiling]
...
(t/ann ^:no-check clojure.core.typed.rt.common.current-impl/assert-clojure [-> Any])
  ...
  (let [e (ex-info msg (merge {:type-error tc-error-parent}
    ...
    (cond
      ...
      (do
        ...
        (or return @(ns-resolve (find-ns 'clojure.core.typed.chk.common.type-rep) '-nothing))))))
  ...
  (let [classname (with-meta (symbol (str (namespace-munge *ns*) "." name)) (meta name))
    ...
    `(t/tc-ignore
       ...
       (deftype ~name [~@fields ~intern-id ~meta-field]
         ...
         clojure.core.typed.chk.common.type-rep/TypeId
         (type-id [_#] ~intern-id)
