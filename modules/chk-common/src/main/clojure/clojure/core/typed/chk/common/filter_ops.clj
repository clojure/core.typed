(ns ^:skip-wiki clojure.core.typed.chk.common.filter-ops
...
(defn- subtype?-var []
  (let [v (ns-resolve (find-ns 'clojure.core.typed.chk.common.subtype) 'subtype?)]
...
;  (prn "compact")
;  (prn "props" (map clojure.core.typed.chk.common.parse-unparse/unparse-filter props))
;  (prn "or?" or?)
  (let [tf-map (atom {})
        ntf-map (atom {})]
...
(defn -or [& args]
  ...
  (letfn [(mk [& fs]
          ...
          (distribute [args]
            ;(prn "distribute:" (map clojure.core.typed.chk.common.parse-unparse/unparse-filter args))
      ...
      (if (empty? fs)
        (cond
          ...
          (let [{atomic true not-atomic false} (group-by atomic-filter? result)
                ;_ (prn "not-atomic" (map clojure.core.typed.chk.common.parse-unparse/unparse-filter not-atomic))
                not-atomic* (for [p not-atomic
                                  :when (not-any? (fn [a] (implied-atomic? p a)) atomic)]
                              p)]
            ;(prn "not-atomic*" (map clojure.core.typed.chk.common.parse-unparse/unparse-filter not-atomic*))
