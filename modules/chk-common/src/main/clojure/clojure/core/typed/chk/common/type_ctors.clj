(ns ^:skip-wiki clojure.core.typed.chk.common.type-ctors
  (:refer-clojure :exclude [defrecord])
  (:require [clojure.core.typed.chk.common.utils :as u :refer [p]]
            [clojure.core.typed.chk.common.impl-protocols :as p]
            [clojure.core.typed.chk.common.type-rep :as r :refer [ret-t]]
...
(t/typed-deps clojure.core.typed.chk.common.name-env)
...
(t/tc-ignore
  (create-ns 'clojure.core.typed.chk.common.cs-rep)
  (alias 'cr 'clojure.core.typed.chk.common.cs-rep)
  )

(t/tc-ignore
(defn- unparse-type-var []
  (let [v (ns-resolve (find-ns 'clojure.core.typed.chk.common.parse-unparse)
                      'unparse-type)]
  ...
  (defn- unparse-type [t]
    (@(unparse-type-var) t))
  (defn- parse-type [t]
    ((impl/v 'clojure.core.typed.chk.common.parse-unparse/parse-type) t))
  (defn- check-funapp [& args]
    (apply (impl/v 'clojure.core.typed.chk.common.check/check-funapp) args))
  )
...
(t/tc-ignore
(defn- subtype?-var []
  (let [n (find-ns 'clojure.core.typed.chk.common.subtype)
...
(defn- subtype? [& args]
  (apply (impl/v 'clojure.core.typed.chk.common.subtype/subtype?) args))
  )
...
(defn JSNominal-of
  ...
  ([sym args]
   {:pre [(symbol? sym)
          (every? r/Type? args)]
    :post [(r/Type? %)]}
   (let [p ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/get-jsnominal) sym)]
...
(defn JSNominal-with-unknown-params
  ([sym]
   ...
   (let [t ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/get-jsnominal) sym)
         args (when (r/TypeFn? t)
                (let [syms (TypeFn-fresh-symbols* t)]
                  (most-general-on-variance (:variances t)
                                            (TypeFn-bbnds* syms t))))]
...
(defn JSNominal-method*
  ...
  {:pre [(r/JSNominal? jsnom)
         (symbol? msym)]
   :post [(r/Type? %)]}
  (if-let [t ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/get-method) name poly? msym)]
    t
...
(defn JSNominal-field*
  ...
  {:pre [(r/JSNominal? jsnom)
         (symbol? fsym)]
   :post [(r/Type? %)]}
  (if-let [t ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/get-field) name poly? fsym)]
    t
...
(defn JSNominal-ctor*
  ...
  {:pre [(r/JSNominal? jsnom)]
   :post [(r/Type? %)]}
  (if-let [t ((impl/v 'clojure.core.typed.impl.js.jsnominal-env/get-ctor) name poly?)]
    t
...
(t/tc-ignore
(defn- infer-var []
  (let [v (ns-resolve (find-ns 'clojure.core.typed.chk.common.cs-gen) 'infer)]
...
(t/tc-ignore
(defn- subst-all-var []
  (let [v (ns-resolve (find-ns 'clojure.core.typed.chk.common.subst) 'subst-all)]
...
(defn TypeFn-body* [names typefn]
  ...
  (let [bbnds (TypeFn-bbnds* names typefn)
        ...
        fv-variances (impl/v 'clojure.core.typed.chk.common.frees/fv-variances)
...
(defn resolve-Name [nme]
  {:pre [(r/Name? nme)]}
  (let [resolve-name* (t/var> clojure.core.typed.chk.common.name-env/resolve-name*)]
    (resolve-name* (:id nme))))
...
(t/tc-ignore
(defn- substitute-var []
  (let [v (ns-resolve (find-ns 'clojure.core.typed.chk.common.subst) 'substitute)]
